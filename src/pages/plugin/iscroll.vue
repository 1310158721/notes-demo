<template>
	<div class="container_wrapper">
		<p class="page-title">iscroll 使用总结</p>
...
export default {
	...
	methods: {
		getList () {
			ajaxGet('/iscrollGetList', this.listPostData).then(res => {
		...
		addListItem () {
			ajaxPost('/iscrollAddItem', this.addPostData).then(res => {
				if (res.data.code === 200) {
					this.addAlert = false
					this.getList()
				}
		...
		delListItem (_id) {
			ajaxGet('/iscrollDelItem', {_id: _id}).then(res => {
				if (res.data.code === 200) {
		...
		getSingleItem (_id) {
			ajaxGet('/iscrollSingleItem', {_id: _id}).then(res => {
				if (res.data.code === 200) {
		...
		editListItem (_id) {
			ajaxPost('/iscrollEditItem', {_id: _id, title: this.editPostData.title, content: this.editPostData.content, type: this.editPostData.type}).then(res => {
				if (res.data.code === 200) {
					this.getList()
