<template>
	<div class="container_wrapper">
		<p class="page-title">swiper 使用总结</p>
...
export default {
	...
	methods: {
		getList () {
			ajaxGet('/swiperGetList', this.listPostData).then(res => {
		...
		addListItem () {
			ajaxPost('/swiperAddItem', this.addPostData).then(res => {
				if (res.data.code === 200) {
					this.addAlert = false
					this.getList()
				}
		...
		delListItem (_id) {
			ajaxGet('/swiperDelItem', {_id: _id}).then(res => {
				if (res.data.code === 200) {
		...
		getSingleItem (_id) {
			ajaxGet('/swiperSingleItem', {_id: _id}).then(res => {
				if (res.data.code === 200) {
		...
		editListItem (_id) {
			ajaxPost('/swiperEditItem', {_id: _id, title: this.editPostData.title, content: this.editPostData.content, type: this.editPostData.type}).then(res => {
				if (res.data.code === 200) {
					this.getList()
