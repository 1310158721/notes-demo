<template>
	<div class="container_wrapper">
		<p class="page-title">mvvm 框架的使用总结</p>
		...
		<el-card class="box-card card" v-show="Number.parseInt(listPostData.type) !== 5">
			<div v-if="listInfo">
		...
		<el-card class="box-card card" v-show="Number.parseInt(listPostData.type) === 5">
			<div v-if="listInfo">
			    ...
			    <div v-show="chartsData.length" id="main" style="height:400px;"></div>
			</div>
			<p v-else>该类型暂无数据</p>
		</el-card>
		<div class="alert_wrapper">
			...
			<el-dialog
				...
				<el-form :model="addPostData" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
					<el-form-item label="类型">
						<el-select v-model="addPostData.type" placeholder="请选择">
							<el-option
			...
			<el-dialog
				...
				<el-form :model="editPostData" :rules="rules" ref="edit_ruleForm" label-width="100px" class="demo-ruleForm">
					<el-form-item label="类型">
						<el-select v-model="editPostData.type" placeholder="请选择">
							<el-option
								v-for="item in typeOptions"
								:key="item.type"
								:label="item.label"
...
export default {
	...
	methods: {
		...
		addListItem () {
			ajaxPost('/mvvmAddItem', this.addPostData).then(res => {
				if (res.data.code === 200) {
					this.addAlert = false
					this.getList()
				}
		...
		delListItem (_id) {
			ajaxGet('/mvvmDelItem', {_id: _id}).then(res => {
				if (res.data.code === 200) {
		...
		getSingleItem (_id) {
			ajaxGet('/mvvmGetSinlgeItem', {_id: _id}).then(res => {
				if (res.data.code === 200) {
		...
		editListItem (_id) {
			ajaxPost('/mvvmEditItem', {_id: _id, title: this.editPostData.title, content: this.editPostData.content, type: this.editPostData.type}).then(res => {
				if (res.data.code === 200) {
					this.getList()
		...
		screenToArray (arr) {
			if (arr.length === 0) {
				...
				const type3 = arr.filter(item => item.type === '3').length
				const type4 = type1 + type2 + type3
