<template>
	<div class="container_wrapper">
		<p class="page-title">node 模块的使用总结</p>
		...
		<el-card class="box-card card" v-show="Number.parseInt(listPostData.type) !== 4">
			<div v-if="listInfo">
...
export default {
	...
	methods: {
		getList () {
			ajaxGet('/nodeGetList', this.listPostData).then(res => {
		...
		addListItem () {
			ajaxPost('/nodeAddItem', this.addPostData).then(res => {
				if (res.data.code === 200) {
					this.addAlert = false
					this.getList()
				}
		...
		delListItem (_id) {
			ajaxGet('/nodeDelItem', {_id: _id}).then(res => {
				if (res.data.code === 200) {
		...
		getSingleItem (_id) {
			ajaxGet('/nodeGetSingleItem', {_id: _id}).then(res => {
				if (res.data.code === 200) {
		...
		editListItem (_id) {
			ajaxPost('/nodeEditItem', {_id: _id, title: this.editPostData.title, content: this.editPostData.content, type: this.editPostData.type}).then(res => {
				if (res.data.code === 200) {
					this.getList()
