<template>
	<div class="container_wrapper">
		<p class="page-title">canvas 使用总结</p>
...
export default {
	...
	methods: {
		getList () {
			ajaxGet('/canvasGetList', this.listPostData).then(res => {
		...
		addListItem () {
			ajaxPost('/canvasAddItem', this.addPostData).then(res => {
				if (res.data.code === 200) {
					this.addAlert = false
					this.getList()
				}
		...
		delListItem (_id) {
			ajaxGet('/canvasDelItem', {_id: _id}).then(res => {
				if (res.data.code === 200) {
		...
		getSingleItem (_id) {
			ajaxGet('/canvasSingleItem', {_id: _id}).then(res => {
				if (res.data.code === 200) {
		...
		editListItem (_id) {
			ajaxPost('/canvasEditItem', {_id: _id, title: this.editPostData.title, content: this.editPostData.content, type: this.editPostData.type}).then(res => {
				if (res.data.code === 200) {
					this.getList()
