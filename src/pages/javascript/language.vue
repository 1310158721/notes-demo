...
export default {
	...
	methods: {
		getList () {
			ajaxGet('/jsGetList', this.listPostData).then(res => {
		...
		addListItem () {
			ajaxPost('/jsAddItem', this.addPostData).then(res => {
				if (res.data.code === 200) {
					this.addAlert = false
					this.getList()
				}
		...
		delListItem (_id) {
			ajaxGet('/jsDelitem', {_id: _id}).then(res => {
				if (res.data.code === 200) {
		...
		getSingleItem (_id) {
			ajaxGet('/jsGetSingleItem', {_id: _id}).then(res => {
				if (res.data.code === 200) {
		...
		editListItem (_id) {
			ajaxPost('/jsEditItem', {_id: _id, title: this.editPostData.title, content: this.editPostData.content, type: this.editPostData.type}).then(res => {
				if (res.data.code === 200) {
					this.getList()
