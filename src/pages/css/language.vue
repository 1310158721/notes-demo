<template>
	<div class="container_wrapper">
		...
		<el-card class="box-card card" v-show="Number.parseInt(listPostData.type) !== 6">
			<div v-if="listInfo">
...
export default {
	...
	methods: {
		...
		addListItem () {
			ajaxPost('/cssAddItem', this.addPostData).then(res => {
				if (res.data.code === 200) {
					this.addAlert = false
					this.getList()
				}
		...
		delListItem (_id) {
			ajaxGet('/cssDelItem', {_id: _id}).then(res => {
				if (res.data.code === 200) {
		...
		getSingleItem (_id) {
			ajaxGet('/cssGetSingleItem', {_id: _id}).then(res => {
				if (res.data.code === 200) {
		...
		editListItem (_id) {
			ajaxPost('/cssEditItem', {_id: _id, title: this.editPostData.title, content: this.editPostData.content, type: this.editPostData.type}).then(res => {
				if (res.data.code === 200) {
					this.getList()
		...
		screenToArray (arr) {
			if (arr.length === 0) {
				...
				const type4 = arr.filter(item => item.type === '4').length
